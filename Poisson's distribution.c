...
int main(void)
{
	...
	rho_in = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*Npoints);			//allocating memory
	rho_f_out = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*Npoints);		//allocating memory
	phi_f_in = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*Npoints);			//allocating memory
	phi_out = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*Npoints);		//allocating memory
	fourier_transform = fftw_plan_dft_1d(Npoints, rho_in, rho_f_out, FFTW_FORWARD, FFTW_ESTIMATE); 	//Here we set which kind of transformation we want to perform
	...
	{	phi_out[i] = phi_out[i]/Npoints;
		...
		//Shifting phi value at i=0 to be 0
	}
